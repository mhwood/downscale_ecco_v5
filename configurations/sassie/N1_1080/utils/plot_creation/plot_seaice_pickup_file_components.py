...
def read_pickup_file_to_faces(input_dir):
    ...
    for var_name in global_metadata['fldlist']:
        ...
        var_grid_faces = sf.sassie_n1_compact_to_faces(var_grid,dim=2,Nr=50)
        var_grid = sf.stitch_faces_to_single_grid(var_grid_faces,llc=1080,rows=680)
...
def plot_seaice_pickup_file_components(output_dir,var_names,var_grids):
    ...
    plt.savefig(os.path.join(output_dir,'pickup_seaice_file_components.png'),bbox_inches='tight')
    plt.close(fig)
...
var_names,row_bounds,var_grids,global_metadata = read_seaice_pickup_file_to_faces(input_dir)

plot_seaice_pickup_file_components(output_dir,var_names,var_grids)
