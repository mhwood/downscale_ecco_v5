...
def plot_dv_masks(ecco_path,mask_names):
    ...
    if 'mask_arctic_surface' in masks:
        output_file = os.path.join('..', 'plots', 'diagnostics_vec_surface_masks.png')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", "--masks", action="store", help="List of masks to plot. "
                                                              "Default value is north south west.", default='', dest="masks", type=str, nargs='+',
                        required=False)
